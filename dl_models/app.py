...
MATCH_CONSTANT = 70
MATCH_CONSTANT2 = 80   # lower = stricter match
MIN_MATCH_FRAMES = 3
...
def criminalsearchresults(sid):
    ...
    for c in criminals:
        criminal_id, name, crime_type = c
        folder = os.path.join("static/criminals", str(criminal_id))
        ...
        for img_file in os.listdir(folder):
            ...
            if len(encodings) > 0:
                criminal_embeddings.append(encodings[0])
                criminal_meta.append({
                    "criminal_id": criminal_id,
                    "name": name,
                    "crime_type": crime_type
                })

    if not criminal_embeddings:
        return "No criminal face data available"

    # ===== SEARCH PROCESS =====
    results = {}

    for img_file in os.listdir(search_folder):
        if not img_file.lower().endswith((".jpg", ".jpeg", ".png")):
            continue

        img_path = os.path.join(search_folder, img_file)
        image = face_recognition.load_image_file(img_path)

        search_encodings = face_recognition.face_encodings(image)

        for search_encoding in search_encodings:

            distances = face_recognition.face_distance(criminal_embeddings, search_encoding)

            for i, distance in enumerate(distances):
                if distance < 0.6:  # Deep Learning Threshold

                    cid = criminal_meta[i]["criminal_id"]

                    confidence = round((1 - distance) * 100, 2)

                    if cid not in results or confidence > results[cid]["confidence"]:
                        results[cid] = {
                            "name": criminal_meta[i]["name"],
                            "crime_type": criminal_meta[i]["crime_type"],
                            "confidence": confidence
                        }

    final_results = sorted(results.values(), key=lambda x: x["confidence"], reverse=True)
